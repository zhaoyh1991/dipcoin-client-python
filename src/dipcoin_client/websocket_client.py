...
class WebsocketClient:
    ...
    def set_token(self, token):
        ...
        Inputs:
            - token (user auth token):  onboarding token.
        """
        self.token = token
    ...
    def subscribe_orderbook_depth_streams_by_symbol(self, symbol: MARKET_SYMBOLS, depth="5"):
        """
    ...
    def unsubscribe_orderbook_depth_streams_by_symbol(self, symbol: MARKET_SYMBOLS, depth=""):
        """
            Allows user to subscribe to orderbook depth stream for the desired symbol.
            Inputs:
                - symbol: market symbol of market user wants orderbook depth stream for. (e.g. ETH-PERP)
